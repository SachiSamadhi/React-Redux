...
const employeeAdapter = createEntityAdapter({
  selectId: (emp) => emp.hed_Employee_id, 
});
  ...
  endpoints: (builder) => ({
    ...
    readEmployees: builder.query({
      ...
      transformResponse: (res) => {
        ...
        return employeeAdapter.setAll(initialState, res.ResultSet || []);
      },
      providesTags: (result) =>
        result?.ids
    ...
    getLocationsByDepartment: builder.query({
      query: (deptId) =>
        deptId ? `Employee/GetLocationsByDepartment?deptId=${deptId}` : skipToken,
    }),
...
// ------------------- SELECTORS -------------------
export const selectReadEmployeeResult = EmployeeService.endpoints.readEmployees.select();
export const selectReadEmployeeData = createSelector(
